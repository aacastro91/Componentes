...
/**
 * Description of aCombo
 *
 * @author adm01
 */
class aCombo extends TCombo {

    function __construct($name) {
        parent::__construct($name);
        $this->{'class'} = 'tcombo form-control';
        TPage::include_css('app/lib/Components/Standard/aCombo.css');
        $this->setSize(-1);
        $this->defaultOption = '';
    }
    ...
    public function show() {
        ...
        if (parent::getEditable()) {
			...
			$this->tag->{'class'} = 'form-control tcombo_disabled'; // CSS
        }
        // shows the combobox
        $this->tag->show();
    }
